...
export async function DELETE(
  ...
  try {
    ...
    if (evt.organizer_id !== auth.userId) {
      return NextResponse.json({ error: "You can only delete your own events" }, { status: 403 });
    }

    // Only allow deletion if event is completed
    if (evt.status !== "completed") {
      return NextResponse.json(
        { error: "Can only delete completed events" },
        { status: 400 }
      );
    }
    ...
    const { error: delErr } = await auth.supabase.from("events").delete().eq("id", id);
