...
import { buildEventAnalytics } from "@/lib/event-analytics-server";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const analytics = await buildEventAnalytics(auth.supabase, auth.userId);
    return NextResponse.json({ data: analytics }, { status: 200, headers: NO_CACHE });
  } catch (err: unknown) {
    console.error("GET /api/vendor/events/analytics error:", err);
    return NextResponse.json(
      { error: (err as Error)?.message || "Failed to fetch event analytics" },
      { status: 500 }
    );
  }
}
