...
export async function GET(
  ...
  if (!id) return NextResponse.json({ error: "League ID required" }, { status: 400 });
  try {
    ...
    return NextResponse.json(
      { error: (e as Error)?.message || "Failed to fetch league" },
...
export async function DELETE(
  ...
  if (!id) return NextResponse.json({ error: "League ID required" }, { status: 400 });
  try {
    const { data: league, error: fe } = await auth.supabase
      .from("sim_leagues")
      .select("id, organizer_id")
      .eq("id", id)
      .single();
    if (fe || !league) return NextResponse.json({ error: "League not found" }, { status: 404 });
    if (league.organizer_id !== auth.userId) {
      return NextResponse.json({ error: "You can only delete your own leagues" }, { status: 403 });
    }

    // Check if league has any associated events
    ...
    if (events && events.length > 0) {
      return NextResponse.json(
        { error: "Cannot delete league with associated events. Delete events first." },
        { status: 400 }
      );
    }

    const { error } = await auth.supabase.from("sim_leagues").delete().eq("id", id);
    if (error) throw error;
    return NextResponse.json({ success: true }, { status: 200, headers: NO_CACHE });
  } catch (e: unknown) {
    return NextResponse.json(
      { error: (e as Error)?.message || "Failed to delete league" },
      { status: 500 }
    );
  }
}
